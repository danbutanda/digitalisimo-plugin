...
class Digitalisimo_AI_Images {
	...
	public static function generate( $request ) {
		...
		wp_update_post( array( 'ID' => $attachment, 'post_title' => $title, 'post_name' => sanitize_title( $title ), 'post_mime_type' => 'image/webp' ) ); update_post_meta( $attachment, '_wp_attachment_image_alt', $alt ); update_post_meta( $attachment, '_digitalisimo_ai_generated', '1' ); update_post_meta( $attachment, '_digitalisimo_image_format', 'webp' ); if ( $post_id ) set_post_thumbnail( $post_id, $attachment ); $size = wp_get_attachment_image_src( $attachment, 'full' );
		return rest_ensure_response( array( 'attachment_id' => $attachment, 'post_id' => $post_id, 'url' => wp_get_attachment_url( $attachment ), 'title' => $title, 'alt' => $alt, 'mime_type' => 'image/webp', 'format' => 'webp', 'quality' => self::WEBP_QUALITY, 'width' => (int) ( $size[1] ?? self::OUTPUT_WIDTH ), 'height' => (int) ( $size[2] ?? self::OUTPUT_HEIGHT ) ) );
	}
