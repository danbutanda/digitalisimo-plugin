...
class Digitalisimo_AI_Images {
	...
	/** Alterna recursos visuales para que dos solicitudes del mismo artículo no dupliquen la misma escena. */
	private static function creative_direction() { $directions = array( 'fotografía editorial a ras de suelo con luz matinal lateral', 'escena arquitectónica minimalista con perspectiva amplia', 'bodegón conceptual táctil con sombras geométricas', 'narrativa documental cálida con profundidad de campo corta', 'ilustración editorial contemporánea con materiales físicos y textura sutil', 'escena nocturna elegante con luz ambiental suave', 'composición cenital ordenada con objetos reales relacionados al tema', 'retrato ambiental natural en un espacio de trabajo auténtico' ); return $directions[ wp_rand( 0, count( $directions ) - 1 ) ]; }
	...
	private static function script( $endpoint, $nonce, $article ) {
		...
		$placement = $article ? 'var placeTool=function(){var tool=document.getElementById("digitalisimo-ai-article-image-tool"),featured=document.querySelector("#postimagediv .inside,.editor-post-featured-image,.edit-post-featured-image");if(!tool)return false;if(!featured){var panels=document.querySelectorAll(".components-panel__body,.edit-post-sidebar__panel-tab");for(var i=0;i<panels.length;i++){var heading=panels[i].querySelector(".components-panel__body-title,h2,h3,button");if(heading&&/^(Imagen destacada|Featured image)$/i.test((heading.textContent||"").trim())){featured=panels[i];break;}}}if(!featured)return false;var target=featured.querySelector(".components-panel__body-content,.editor-post-featured-image__container")||featured;if(tool.parentNode!==target)target.appendChild(tool);var box=document.getElementById("digitalisimo-ai-article-image");if(box)box.style.display="none";return true;};if(!placeTool()){var observer=new MutationObserver(function(){if(placeTool())observer.disconnect();});observer.observe(document.body,{childList:true,subtree:true});setTimeout(function(){observer.disconnect();},300000);}' : '';
