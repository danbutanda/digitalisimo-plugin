...
class Digitalisimo_AI_Images {
	...
	private static function script( $endpoint, $nonce, $article ) {
		...
		$placement = $article ? 'var tool=document.getElementById("digitalisimo-ai-article-image-tool"),featured=document.querySelector("#postimagediv .inside");if(tool&&featured){featured.appendChild(tool);var box=document.getElementById("digitalisimo-ai-article-image");if(box)box.style.display="none";}' : '';
		echo '<script>document.addEventListener("DOMContentLoaded",function(){var b=document.getElementById("' . esc_js( $button ) . '"),s=document.getElementById("' . esc_js( $status ) . '"),r=document.getElementById("' . esc_js( $result ) . '");' . $placement . 'if(!b)return;b.addEventListener("click",function(){var post=' . $post . ',alt=' . $alt . ',prompt=' . $prompt . ';b.disabled=true;s.textContent="Generando imagen destacada…";fetch("' . esc_js( $endpoint ) . '",{method:"POST",headers:{"Content-Type":"application/json","X-WP-Nonce":"' . esc_js( $nonce ) . '"},body:JSON.stringify({post_id:post,prompt:prompt,image_alt:alt})}).then(function(x){return x.json()}).then(function(x){if(x.url){s.textContent="Imagen destacada lista en WebP.";r.innerHTML="<p><a href=\""+x.url+"\" target=\"_blank\" rel=\"noopener\"><img style=\"max-width:640px;height:auto\" src=\""+x.url+"\" alt=\"Imagen generada\"></a></p>"}else s.textContent=x.message||"No fue posible generar la imagen."}).catch(function(){s.textContent="Error de conexión."}).finally(function(){b.disabled=false})})})</script>';
	}
}
