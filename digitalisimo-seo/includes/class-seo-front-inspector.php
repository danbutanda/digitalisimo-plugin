...
class Digitalisimo_Integrations_SEO_Front_Inspector {
	...
	public static function render() {
		...
		$warnings = array(); if ( 1 !== count( $titles ) ) $warnings[] = 'Debe haber un solo title; se detectaron ' . count( $titles ) . '.'; if ( count( $meta['description'] ?? array() ) > 1 ) $warnings[] = 'Hay más de una meta description.'; if ( count( $canonicals ) > 1 ) $warnings[] = 'Hay más de una URL canonical.'; if ( $invalid_schema ) $warnings[] = 'Hay ' . $invalid_schema . ' bloque(s) Schema con JSON inválido.'; if ( $duplicate_schema_ids ) $warnings[] = 'Hay IDs de Schema repetidos; otro plugin o el tema puede estar emitiendo la misma entidad.'; if ( self::configured( 'seo_open_graph' ) && empty( $meta['og:title'] ) ) $warnings[] = 'Open Graph está activo en Digitalisimo, pero og:title no se detectó en el front.'; if ( self::configured( 'seo_schema_enabled' ) && ! $schemas ) $warnings[] = 'Schema está activo en Digitalisimo, pero no se detectó JSON-LD en el front.';
