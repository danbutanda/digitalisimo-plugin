...
| Acceso oculto al escritorio | ~ | Ruta privada configurable en SEO → General: sirve wp-login.php desde el slug elegido y deja sin respuesta wp-login.php, /wp-admin, /admin y /dashboard para visitas sin sesión; con sesión iniciada /wp-admin no cambia. Salida de emergencia por constante `DIGITALISIMO_HIDE_LOGIN_DISABLE`. Falta validación runtime en WordPress público. |
...
Última validación estructural: los ZIP de SEO 1.0.28, Ecommerce 1.0.10, Geolocalización 1.0.8, AI/Chatbot 1.0.13 y Hosting 1.0.4 contienen su archivo principal, declaran la versión esperada y superan `unzip -t`. La prueba runtime en WordPress individual/Multisite continúa pendiente.
