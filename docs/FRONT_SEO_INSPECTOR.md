...
El informe separa la respuesta técnica en grupos: SEO del HTML (`title`, description, keywords, canonical, robots, hreflang, idioma, charset y paginación), un índice ordenado de todos los encabezados `H1` a `H6`, imágenes y enlaces, metadatos Open Graph/X, bloques Schema JSON-LD y redirecciones. El índice muestra el tipo y el texto de cada título publicado. Además conserva un desplegable con el `head` recibido, escapado para que se pueda revisar sin ejecutar código del sitio inspeccionado.
